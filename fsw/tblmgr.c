...
** This a template file however it has not been setup to allow an automated
** keyword replacement. See notes below and _template_.h for template
** guidelines.
**
############################# End Template Block ############################*/
/* 
...
**
** Notes
**   1. The app-specific changes have comment blocks starting
**      pound signs (#).
**
** References:
...
static void XMLCALL ExObjTblStartElement(void *data, const char *el, const char **attr)
{
   ...
   if (strcmp(el,TBLMGR_XML_EL_PKT_ENTRY) == 0)
   {
      ...
      if (ProcessedAttr == 0)
      {
        ...
        TblMgr->ExObjTbl.Local.Entry[Index].Data1 = Data1;
        TblMgr->ExObjTbl.Local.Entry[Index].Data2 = Data2;
        TblMgr->ExObjTbl.Local.Entry[Index].Data3 = Data3;
        TblMgr->ExObjTbl.Modified[Index] = TRUE;
      }
      else
      {
         TblMgr->ExObjTbl.AttrErrCnt++;
      }
